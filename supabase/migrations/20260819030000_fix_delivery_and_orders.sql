-- Fix delivery_fees table
DROP TABLE IF EXISTS public.delivery_fees;
...
-- Add commune to orders
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS commune TEXT DEFAULT '';

-- Update create_order RPC
DROP FUNCTION IF EXISTS public.create_order(text, text, text, text, jsonb);

CREATE OR REPLACE FUNCTION public.create_order(
  p_customer_name text, p_phone text, p_wilaya text, p_commune text, p_address text, p_items jsonb
...
BEGIN
  ...
  INSERT INTO public.orders (customer_name, phone, wilaya, commune, address, total)
  VALUES (p_customer_name, p_phone, p_wilaya, p_commune, p_address, v_total)
